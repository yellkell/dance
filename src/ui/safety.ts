...
export function safetyKey(rows: SafetyRow[], more: number): string {
  const people = rows
    ...
    .join(';');
  return `${people}#${more}#${voiceEnabled() ? 1 : 0}#${isVoiceMuted() ? 1 : 0}#${isVoiceCapturing() ? 1 : 0}`;
}
...
export function voiceButtons(x: number, y: number, w: number): PanelButton[] {
