 ...
 *
 * SLUGFEST's organ, vendored home into the lineage that spawned it. The
 * creature here was always POSED (authored anchor targets, the boss's
 * dance vocabulary); this drives it live from a human in a headset:
 *
 *   head + two hands  ──►  20 anchor targets  ──►  the sim's springs,
 *                                                   leash, swell, wobble
 *
 * The mapping is deliberately 1:1 — no retarget scaling. Your fist blob is
 * pinned EXACTLY where your controller is (the glowstick hand must be your
 * hand), your head anchor rides your headset, and everything unmeasured is
 * DERIVED:
 *
 *  - trunk (neck/chest/belly/pelvis) hangs off the head at fixed fractions
 *    of your live head height, leaning from the pelvis up — duck a sweep
 *    and the whole column squashes; bow and the gel pours forward after you;
 *  - shoulders ride the neck, turn a little with your gaze, SHRUG toward a
 *    raised glowstick and reach after a thrown hand;
 ...
 *    hands-up grooving folds its elbows in, a full reach straightens the
 *    rope;
 *  - legs keep the stance under the body's ROOT, and the root chases your
 *    head across the deck with oozy lag (GelCreature.moveTo), so every
 *    sidestep dodge reads as the skirt dragging after you — footwork for
 *    free.
 *
 ...
 * forever, and an invisible dancer is a lost set.
 */
...
/** Solved elbows keep this far (centre to centre) from the head blob —
 *  swelled elbow radius + headset offset + room for spring overshoot. */
const ELBOW_HEAD_CLEAR = 0.4;
...
export class EmbodyRig {
  ...
  drive(pose: TrackedPose): void {
    ...
    // A collapsed or resting body isn't driven: the glob/KO poses own it,
    // and stale offsets would haunt the morph.
    ...
    // a first-person green-out SLUGFEST bought with blood.)
    const neckY = H - 0.18;
    const chestY = H - 0.33;
    ...
    /* ── legs: the stance under the root, squashing with a duck ───────── */
    ...
    // root's own motion — a hand is never a frame behind its controller.
    sim.clearPins();
    c.puppetPins.length = 0;
    ...
    // Fist swell rides hand speed; a hand held near the face fattens a
    // touch. Elbows take a share of the swell so a fast arm reads as one
    // thick rope (the cohesion swell floors this — whichever asks for
    // more wins).
    const spL = Math.min(1, Math.max(pose.speedL, 0) / BODY.maxHandSpeed);
    const spR = Math.min(1, Math.max(pose.speedR, 0) / BODY.maxHandSpeed);
    ...
    // full-extension reach stays a rope of gel, not beads (shader follows).
  ...
  private arm(
    ...
    // vector whose length grows with arm slack — the fold of a raised
    // glowstick.
    _d.copy(_f).sub(_s);
    ...
    // The elbow never crowds the wearer's face — a RAVE RAID lesson the
    // boxing lineage never needed: boxing keeps elbows low and forward,
    // but a groove swap throws the hand HIGH and the solved elbow rides
    // up beside the temple, where spring overshoot can carry the live
    // blob into the eyes. Push the target radially clear of the head
    // (a real elbow can't occupy your skull either); the springs get the
    // same margin to overshoot into.
    _d.copy(_elbow).sub(_head);
    const headGap = _d.length();
    if (headGap < ELBOW_HEAD_CLEAR && headGap > 1e-4) {
      _elbow.copy(_head).addScaledVector(_d, ELBOW_HEAD_CLEAR / headGap);
    }
